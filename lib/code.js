...
const reactiveFunc = (func, length) => {
...
const doMember = (obj, optional, prop) => {
    ...
    if (Array.isArray(obj)) {
        if (["join", "includes", "indexOf", "lastIndexOf"].includes(prop)) {
            return res.bind((0, streams_1.resolve)(obj, true));
        }
        if ([
            "copyWithin",
            "fill",
...
const doCall = (func, optional, ...args) => {
    ...
    if (["bound map", "bound reduce"].includes(func.name)) {
        return func(...args.map((a, i) => (i === 0 ? (0, streams_1.resolve)(a) : a)));
    }
    if (["bound flatMap", "bound sort"].includes(func.name)) {
        return func(...args.map((a) => {
            const v = (0, streams_1.resolve)(a);
            return typeof v === "function" ? (...x) => (0, streams_1.resolve)(v(...x)) : v;
        }));
    }
    return func(...args.map((a) => {
        const v = (0, streams_1.resolve)(a, true);
        return typeof v === "function" ? (...x) => (0, streams_1.resolve)(v(...x), true) : v;
    }));
};
exports.default = (code) => {
    ...
    return {
        ...
        run: (getValue) => (0, streams_1.derived)(() => func(getValue, doMember, doCall, streams_1.resolve), code),
    };
};
