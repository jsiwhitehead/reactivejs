...
const mergeUsedVars = (...values) => Object.fromEntries(Object.keys(values.reduce((a, b) => ({ ...a, ...b }), {})).map((k) => [
    k,
    [...new Set(values.flatMap((v) => v[k] || []))],
]));
const getUsedVars = (node, ignore, depend) => {
    if (node.type === "value") {
        return mergeUsedVars({ [depend]: node.vars.filter((n) => n[0] !== "$" && !ignore.has(n)) }, ...node.values.map((n) => getUsedVars(n, ignore, depend)));
    }
        ...
        !node.capture) {
        return mergeUsedVars(...node.items.map((n) => getUsedVars(n, ignore, depend)));
    }
    return {};
};
const createGroup = (type, items, capture) => {
    const groupKeys = new Set(items
        .filter((n) => ["assign", "merge"].includes(n.type))
        .map((n) => n.key || ""));
    return {
        type,
        items,
        assignItems: Object.fromEntries(items
...
s.addAttribute("ast", {
    ...
    brackets: (_1, a, _2, b, _3, _4) => createGroup("brackets", b.ast, a.sourceString === "~"),
    object: (_1, a, _2, b, _3, _4) => createGroup("object", b.ast, a.sourceString === "~"),
    array: (_1, a, _2, b, _3, _4) => createGroup("array", b.ast, a.sourceString === "~"),
    items: (a, _1, _2) => a.ast,
    join: (_1, _2, _3) => null,
    merge: (a, _1, b, _2, c) => ({
        type: "merge",
    ...
    plainblock: (_1, a, _2, b, _3, _4, c, _5) => createGroup("block", [...b.ast, ...c.ast], a.sourceString === "~"),
    block: (_1, a, _2, b, _3, _4, c, _5, _6, _7) => createGroup("block", [{ type: "assign", key: "", value: a.ast }, ...b.ast, ...c.ast], true),
    plainblockclosed: (_1, a, _2, b, _3, _4) => createGroup("block", b.ast, a.sourceString === "~"),
    blockclosed: (_1, a, _2, b, _3, _4) => createGroup("block", [{ type: "assign", key: "", value: a.ast }, ...b.ast], true),
    bmerge: (a, b, _1, c) => ({
        type: "merge",
    ...
    value: (a) => {
        ...
        for (let i = 1; i < ast.length; i++) {
            if ((ast[i - 1].type !== "string" || /[^\s!-]$/.test(ast[i - 1].value)) &&
                ["brackets", "array"].includes(ast[i].type)) {
                if (ast[i].type === "brackets") {
                    result.push({ type: "string", value: "(..." }, { ...ast[i], type: "array" }, { type: "string", value: ")" });
                }
                else {
        ...
        const code = result
            ...
            .join("");
        return { type: "value", values, ...(0, code_1.default)(code) };
    },
