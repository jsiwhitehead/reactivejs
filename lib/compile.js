...
const doCall = (func, args) => {
    ...
    return func(...args.map((a) => {
        const v = (0, streams_1.get)(a, true);
        return typeof v === "function" ? (...x) => (0, streams_1.get)(v(...x), true) : v;
    }));
};
const compileNode = (node, getVar, noTrack) => {
    ...
    if (node.type === "value") {
        ...
        const getValue = (name) => {
            ...
            if (name === noTrack) {
                return (0, streams_1.derived)(() => (0, streams_1.get)(getVar(name), true, true));
            }
            return getVar(name);
        };
      ...
      return (getValue, doMember, doCall, resolve, resolveDeep) => {
        ...
        return (0, streams_1.derived)(() => func(getValue, doMember, doCall, streams_1.get, (x) => (0, streams_1.get)(x, true)));
    }
    if (node.type === "func") {
        const result = (...args) => {
    ...
    const result = () => {
        ...
        for (const n of items.filter((n) => !((0, util_1.isObject)(n) && ["assign", "merge"].includes(n.type)))) {
            if ((0, util_1.isObject)(n) && n.type === "unpack") {
                const v = (0, streams_1.get)(compileNode(n.value, getVar, noTrack));
        ...
        for (const { key, value } of mergeItems.filter((n) => n.value)) {
            ...
            (0, streams_1.get)((0, streams_1.derived)(() => {
                const res = (0, streams_1.get)(source, true);
                if (!first)
                    target(res);
                first = false;
            }));
