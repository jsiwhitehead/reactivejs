...
const compileNode = (node, getVar, noTrack) => {
    ...
    if (node.type === "value") {
        ...
        const getValue = (name) => {
            ...
            if (name === noTrack) {
                return (0, streams_1.derived)(() => (0, util_1.resolve)(getVar(name), true, true));
            }
            return getVar(name);
        };
      ...
      return (getValue, doMember, doCall, resolve, resolveDeep) => {
        ...
        return (0, streams_1.derived)(() => func(getValue, doMember, doCall, util_1.resolve, (x) => (0, util_1.resolve)(x, true)));
    }
    if (node.type === "func") {
        const result = (...args) => {
    ...
    const result = () => {
        ...
        for (const n of items.filter((n) => !((0, util_1.isObject)(n) && ["assign", "merge"].includes(n.type)))) {
            if ((0, util_1.isObject)(n) && n.type === "unpack") {
                const v = (0, util_1.resolve)(compileNode(n.value, getVar, noTrack));
        ...
        for (const { key, value } of mergeItems.filter((n) => n.value)) {
            ...
            (0, util_1.resolve)((0, streams_1.derived)(() => {
