    ...
    if (node.type === "value") {
        ...
        const getValue = (name) => {
            if (name[0] === "$") {
                ...
                if (!compiled[index]) {
                    compiled[index] = compileNode(createData, create, node.values[index], getVar, noTrack);
                }
      ...
      return (getValue, doMember, doCall, resolve, resolveDeep) => {
        ...
        return create(streamMap((get, create) => func(getValue, doMember, (func, args) => doCall(get, create, func, args), get, (x) => get(x, true))));
    }
    if (node.type === "func") {
...
const compileBlock = (createData, create, { type, tag, items }, getVar, noTrack) => {
    const result = (create, get) => {
        ...
        for (const n of items.filter((n) => !((0, util_1.isObject)(n) && ["assign", "merge"].includes(n.type)))) {
            if ((0, util_1.isObject)(n) && n.type === "unpack") {
                const v = get(compileNode(createData, create, n.value, getVar, noTrack));
        ...
        const newGetVar = (name, captureUndef = type === "block" ? true : undefined) => {
            if (values[name] !== undefined)
                return values[name];
            if (assignItems[name]) {
                return (values[name] = compileNode(createData, create, assignItems[name], (n, c) => n === name &&
        ...
        const content = partialContent.map((x) => x.compiled
            ? x.value
    ...
    if (!items.some((n) => (0, util_1.isObject)(n) && n.type === "unpack")) {
