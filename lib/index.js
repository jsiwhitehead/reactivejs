...
import { createComputed, createEffect, createMemo, createRoot, createSignal, untrack, } from "./signal";
...
export const resolve = (node, deep = false) => {
    if (isReactive(node))
        return resolve(node(), deep);
    if (!deep)
        return node;
    if (Array.isArray(node))
        return node.map((x) => resolve(x, true));
    if (isObject(node)) {
        return Object.keys(node).reduce((res, k) => ({ ...res, [k]: resolve(node[k], true) }), {});
    }
    return node;
};
...
const updateTree = (tree) => {
    ...
    const walkNode = (node, parent, prop) => {
        if (!isObject(node) || typeof node.type !== "string")
            return node;
        const walked = mapObject(node, (v, k) => Array.isArray(v)
...
const compileNode = (node, getVar, noTrack) => {
    ...
    if (Array.isArray(node)) {
        ...
        const tree = acorn.parse(code, { ecmaVersion: 2022 });
      ...
      return function(getValue, resolve, resolveDeep) {
        ...
        return createDerived(() => func(getValue, resolve, (x) => resolve(x, true)), newCode.length > 1);
    }
    if (node.type === "func") {
        const result = (...args) => {
    ...
    const result = () => {
        ...
        const newGetVar = (name, captureUndef = node.type === "block" ? true : undefined) => {
        ...
        const tempContent = [];
        for (const n of node.items.filter((n) => !(isObject(n) && ["assign", "merge"].includes(n.type)))) {
            if (isObject(n) && n.type === "unpack") {
                ...
                tempContent.push(...(block.items || []).map((x) => ({ compiled: true, value: x })));
            }
            else {
        ...
        const mergeItems = node.items.filter((n) => isObject(n) && n.type === "merge" && n.value);
        ...
        const content = tempContent.map((x) => x.compiled ? x.value : compileNode(x.value, newGetVar, noTrack));
...
export default (script, library = {}, update) => {
    createRoot(() => {
        const compiled = compileNode(parse(script), (name) => library[name] || null, null);
        createEffect(() => {
            update(compiled);
        });
    });
